...
from cosmo.app.convert_app import (  # app layer API
    ConvertConfig,
    run_convert,
)
...
def test_run_convert_writes_csv_and_sets_csv_path(tmp_path: Path):
    ...
    # Minimal calibration file path (convert_app passes this to converter if provided)
    calib = {
        "fps": 30.0,
        "image_width": 3840,
        "image_height": 2160,
    ...
    cfg = ConvertConfig(
        ...
        fps=None,
        write_csv=True,
        write_mcap=False,  # keep core tests independent of optional MCAP deps
        swap_xy=False,
        flip_x=False,
        flip_y=False,
        xy_offset=(0.0, 0.0),
    ...
    logs: list[str] = []
    result = run_convert(cfg, log_fn=logs.append)  # same signature used by CLI
    ...
    # csv_path is only set if the file exists (summary filters non-existent files)
...
def test_run_convert_no_csv_results_in_none_csv_path(tmp_path: Path):
    ...
    cfg = ConvertConfig(
        ...
        out_dir=str(runs_dir),
        run_name="wb_no_csv",
    )  # relies on defaults for other fields
