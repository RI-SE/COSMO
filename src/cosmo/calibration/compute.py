...
# ------------------------------------------------------------------------------------------
# WGS84 -> ECEF -> ENU (copied/refactored from scripts/compute_calibration.py)
# ------------------------------------------------------------------------------------------
A = 6378137.0
F = 1 / 298.257223563
E_SQ = F * (2 - F)
...
# ------------------------------------------------------------------------------------------
# OpenDRIVE geoReference parser (+lat_0, +lon_0)
# ------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------
# Agg-only plotting helpers (thread-safe; avoids pyplot/Qt backends)
# ------------------------------------------------------------------------------------------
def _save_residuals_plot_agg(
    residuals_png_path: str,
    world: np.ndarray,
    pred: np.ndarray,
    names_used: List[str],
) -> None:
    ...
    ax.set_title("Ground-plane residuals after homography fit")
    ax.scatter(world[:, 0], world[:, 1], c="tab:green", s=50, label="Ground truth")
    ax.scatter(pred[:, 0], pred[:, 1], c="tab:orange", s=40, label="Projected")

    for i, n in enumerate(names_used):
        ax.arrow(
            pred[i, 0],
            pred[i, 1],
            world[i, 0] - pred[i, 0],
        ...
        ax.text(world[i, 0] + 0.2, world[i, 1] + 0.2, n, fontsize=9)
    ...
    fig.savefig(residuals_png_path, dpi=150)


def _save_overlay_plot_agg(
    ...
    ax.imshow(img)
    ax.set_title("Visual markers projected onto image")

    for n, u, v in pts_img:
        if 0 <= u < w and 0 <= v < h:
            ax.scatter([u], [v], c="yellow", s=40, marker="o")
            ax.text(
                u + 5,
                v - 5,
                n,
            ...
            uc = min(max(u, 5), w - 5)
            vc = min(max(v, 5), h - 5)
            ax.scatter([uc], [vc], c="red", s=40, marker="x")
            ax.text(
                uc + 5,
                vc - 5,
                f"{n} (off-img)",
    ...
    fig.savefig(overlay_png_path, dpi=150)
...
def compute_calibration(
    ...
    *,
    ...
    If lat/lon is used, lat0/lon0 are read from OpenDRIVE <geoReference>.
    """
    ...
    if have_latlon:
        ...
        if origin_lat0 is not None and origin_lon0 is not None:
            ...
            try:
                ...
                # 3) Fallback: mean marker lat/lon
...
def write_calibration_outputs(
    ...
    *,
    ...
    All output paths are explicit (no CWD dependence).
    """
    H = computation.H
    ...
    # Residual plot (Agg-only, no pyplot)
    if residuals_png_path:
        try:
            _save_residuals_plot_agg(residuals_png_path, world, pred, computation.names_used)
        except Exception:
            residuals_png_path = None

    # Overlay markers onto image (Agg-only, no pyplot)
    if overlay_png_path and image_path and Path(image_path).is_file():
        try:
            ...
            _save_overlay_plot_agg(overlay_png_path, img, pts_img)
        except Exception:
            overlay_png_path = None

    # Optional OpenLABEL validation (count only)
